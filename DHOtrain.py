...
from utils.DHOloss_utils import l1_loss, ssim, compute_gradient, update_lambda_schedule, compute_robust_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    print("variable_sh_bands:",args.variable_sh_bands)
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        update_lambda_schedule(iteration, opt.iterations, opt, args)
        loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * Lssim + Lalpha_regul * args.lambda_alpha_regul
        
        if iteration >= opt.densify_until_iter:
            robust_loss = compute_robust_loss(image, gaussians, visibility_filter, gt_image, iteration, opt, args)
            loss +=  robust_loss 

        if iteration >= fine_tune_start:
            L_grad = F.l1_loss(compute_gradient(image), compute_gradient(gt_image))
            t = iteration / opt.iterations
            lambda_grad = args.grad
            loss +=  lambda_grad*L_grad*t
        ...
        with torch.no_grad():
            ...
            # args.mercy_interval=10   densification_interval=100   fine_tune_start=27000   densify_until_iter=15000   opacity_reset_interval=3000
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cull_SH", nargs="+", type=int, default=[15_000])
